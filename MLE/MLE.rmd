...
Next, we step through the optimization algorithm through a few steps manually so we can better understand how our coefficients get updated as we minimize our negative log-likelihood.
...
Next, we step through the process iteratively before proceeding to a while loop. In the code below, we: (1) save the current beta coefficients separately as the old, (2) multiply our learning rate by the first derivative, obtaining the values at which we nudge our coefficients, either positively or negatively, (3) update our current beta by subtracting the error, (4) use the new betas to update our negative log-likelihood, and finally (5) calculate the euclidean distance between our beta estimates which we will use to stop iterating once we reach a specified tolerance.
...
In this section, we utilize a while loop to execute our numerical optimization using gradient descent. This while loop will continue until the euclidean distance between our $\beta$s stop updating past our specified tolerance.
...
As we can see, we have: (1) minimized our negative log-likelihood, and (2) found our estimates of beta iteratively until we reached our specified tolerance (0.000001).
...
1. We remove the negative sign from likelihood function and its gradient.
...
As we can see, we find can find identical results either through maximization or minimization. Further, we can see that BFGS outperforms gradient descent.
